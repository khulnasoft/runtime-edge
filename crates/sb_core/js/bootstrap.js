...
globalThis.bootstrapSBEdge = (opts, isUserWorker, isEventsWorker, version) => {
	...
	ObjectDefineProperty(globalThis, 'KHULNASOFT_VERSION', readOnly(String(version)));
	...
	ObjectDefineProperties(denoOverrides, {
		...
		version: getterOnly(() => ({
			deno: `khulnasoft-runtime-edge-${globalThis.KHULNASOFT_VERSION}`,
			v8: '11.6.189.12',
			typescript: '5.1.6',
		})),
	});
	...
	if (isEventsWorker) {
		...
		delete globalThis.EdgeRuntime;
		ObjectDefineProperties(globalThis, {
			EventManager: getterOnly(() => KhulnasoftEventListener),
		});
	}
