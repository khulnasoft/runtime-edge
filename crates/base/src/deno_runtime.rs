...
impl DenoRuntime {
    ...
    pub async fn new(opts: WorkerContextInitOpts) -> Result<Self, Error> {
        ...
        let user_agent = "khulnasoft-runtime-edge".to_string();
...
mod test {
    ...
    async fn test_main_runtime_creation() {
...

        {
            ...
            let runtime_edge_key: deno_core::v8::Local<deno_core::v8::Value> =
                deno_core::serde_v8::to_v8(inner_scope, "EdgeRuntime").unwrap();
            assert!(!global
                .get(inner_scope, runtime_edge_key)
                .unwrap()
                .is_undefined(),);
        }
    }
    ...
    async fn test_os_ops() {
        ...
        assert!(user_rt_execute_scripts
            .unwrap_err()
            .to_string()
            .contains("Spawning subprocesses is not allowed on Khulnasoft Edge Runtime"));
    }
